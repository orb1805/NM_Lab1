...
class SeidelSolveActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_seidel_solve)
    ...
    override fun onClick(p0: View?) {
        ...
        while (coefficient * Matrix.difference(x, xPrev).infNorm() > eps) {
            xPrev = Matrix.copy(x)
            //x = Matrix.sum(Matrix.sum(Matrix.multiply(alpha1, x), Matrix.multiply(alpha2, xPrev)), beta)
            for (i in x.data.indices)
            {
                x.data[i][0] = 0f
                for (j in x.data.indices)
                    x.data[i][0] += alpha.data[i][j] * x.data[j][0]
                x.data[i][0] += beta.data[i][0]
            }
            count++
        }
